    ...
    for (AActor* Actor : World->PersistentLevel->Actors)
    {
        ...
        if (ALocationAnchorActor* Anchor = Cast<ALocationAnchorActor>(Actor))
        {
            UE_LOG(LogTemp, Log,
                ...
                *Anchor->DisplayName.ToString(),
                *Anchor->AnchorID.ToString());

            OutAnchors.Add(TPair<FGuid, FText>(Anchor->AnchorID, Anchor->DisplayName));
        }
    }
...
void FLocationAnchorActorDetails::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
    ...
    [
        ...
        [
            ...
            {
                FString Left, Right;
                if (Item.IsValid() && Item->Split(TEXT("||"), &Left, &Right))
                    return SNew(STextBlock).Text(FText::FromString(Left));
                return SNew(STextBlock).Text(
                    Item.IsValid() ? FText::FromString(*Item) : FText::GetEmpty());
            })
            .Content()
            [
                SNew(STextBlock)
...
// ── Pickers ───────────────────────────────────────────────────────────────
// ИСПРАВЛЕНИЕ: используем ClearSoft<T>() вместо = nullptr,
// чтобы очищался и FSoftObjectPath внутри TSoftObjectPtr
...
void FLocationAnchorActorDetails::OnRegionPicked(const FAssetData& AssetData)
{
    ...
    // Сбрасываем зависимые уровни: очищаем путь полностью
...
void FLocationAnchorActorDetails::RebuildAnchorList()
{
    ...
    if (!Actor->DestinationLink.TargetFloor.IsNull())
    {
        ...
        if (Floor && !Floor->FloorLevel.IsNull())
        {
            ...
            UE_LOG(LogTemp, Log, TEXT("RebuildAnchorList: FloorLevel = '%s'"), *LevelPath.ToString());
        }
        else
        {
            UE_LOG(LogTemp, Warning,
                TEXT("RebuildAnchorList: Floor загружен=%d, FloorLevel пустой"), Floor != nullptr);
        }
    }
    ...
    if (!LevelPath.IsValid() && !Actor->DestinationLink.TargetRegion.IsNull())
    {
        ...
        if (Region && !Region->RegionLevel.IsNull())
        {
            ...
            UE_LOG(LogTemp, Log, TEXT("RebuildAnchorList: RegionLevel = '%s'"), *LevelPath.ToString());
        }
        else
        {
            UE_LOG(LogTemp, Warning,
                TEXT("RebuildAnchorList: Region загружен=%d, RegionLevel пустой"), Region != nullptr);
        }
    }
...
FReply FLocationAnchorActorDetails::OnApplyClicked()
{
    ...
    // Записываем обратную ссылку в целевой якоре
        ...
        if (LevelPackage)
        {
            ...
            if (TargetWorld && TargetWorld->PersistentLevel)
            {
                for (AActor* LevelActor : TargetWorld->PersistentLevel->Actors)
                {
                    ...
                    DestAnchor->DestinationLink.TargetAnchorDisplayName = Actor->DisplayName;

                    // Обратная ссылка ведёт на Floor или Region (в зависимости от владельца)
