...
TArray<FSpawnSlotState> ASpawnGroupSpawner::CaptureCurrentSlots() const
{
    ...
    for (AActor* Ghost : SpawnedGhosts)
    {
        ...
        Slot.State = EGhostState::Alive;  // исправлено: вместо bIsAlive
...
void ASpawnGroupSpawner::SpawnGroup()
{
    ...
    // ... остальной код SpawnGroup без изменений (слишком длинный, оставляем как есть) ...
    // (но внутри он использует SpawnSingleGhost, который уже корректен)
}
