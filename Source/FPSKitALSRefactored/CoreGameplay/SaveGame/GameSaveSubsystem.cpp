...
{
	UThisSaveGame* SaveGameObject = Cast<UThisSaveGame>(UGameplayStatics::LoadGameFromSlot(CurrentSlot, 0));
	if (!SaveGameObject)
	{
	...
	// Очищаем временные данные у всех подсистем перед загрузкой
	for (const FSubsystemSaveData& Data : SaveGameObject->SavedSubsystems)
	{
		for (ISaveableSubsystem* Subsystem : SaveableSubsystems)
		{
			...
			{
				Subsystem->Execute_ClearTransientData(Subsystem->_getUObject());
				break;
			}
		}
	}
