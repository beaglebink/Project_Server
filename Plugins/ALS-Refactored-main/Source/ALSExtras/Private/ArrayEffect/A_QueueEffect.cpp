...
void AA_QueueEffect::DeleteNode(int32 Index)
{
...
void AA_QueueEffect::InsertNode(FName VariableName, int32 Index)
{
	if (NodeArray.Num() == 10)
	{
		return;
...
void AA_QueueEffect::ContainerConcatenate(AA_QueueEffect* ArrayToConcatenate)
{
...
void AA_QueueEffect::ContainerRename(FText NewName)
{
	SetContainerName(NewName);
}

void AA_QueueEffect::ContainerCopy(FText Name, int32 OutLeftIndex, int32 OutRightIndex)
{
	...
	if (AA_QueueEffect* NewArray = GetWorld()->SpawnActor<AA_QueueEffect>(ContainerClass, EndNode->DefaultLocation - GetActorRightVector() * NodeWidth * 2, GetActorRotation()))
	{
...
bool AA_QueueEffect::ParseCommandToSwap(FText Command, FText& PrevName, int32& OutIndex1, int32& OutIndex2)
{
	FString Input = Command.ToString();
...
bool AA_QueueEffect::ParseCommandToInsert(FText Command, FText& PrevName, FName& VariableName, int32& OutIndex)
{
	FString Input = Command.ToString();
	Input.RemoveSpacesInline();
...
bool AA_QueueEffect::ParseCommandToClear(FText Command, FText& PrevName)
{
	FString Input = Command.ToString();
	Input.RemoveSpacesInline();
...
bool AA_QueueEffect::ParseCommandToExtend(FText Command, FText& PrevName, TArray<int32>& OutArray)
{
	OutArray.Empty();
	FString Input = Command.ToString();
	Input.RemoveSpacesInline();
...
bool AA_QueueEffect::ParseCommandToConcatenate(FText Command, int32& OutSize1, int32& OutSize2)
{
	FString Input = Command.ToString();
...
bool AA_QueueEffect::ParseCommandToReset(FText Command, FText& PrevName)
{
	FString Input = Command.ToString();
	Input.RemoveSpacesInline();
...
bool AA_QueueEffect::ParseCommandToRename(FText Command, FText& PrevName, FText& NewName, int32& ArrayNum)
{
...
bool AA_QueueEffect::ParseCommandToCopy(FText Command, FText& PrevName, FText& CopyName, int32& OutLeftIndex, int32& OutRightIndex)
{
	FString Input = Command.ToString();
	Input.RemoveSpacesInline();
	FString Left, Right;
...
void AA_QueueEffect::AttachToCharacterCamera()
{
...
void AA_QueueEffect::DetachFromCharacterCamera()
{
	if (!bIsDetaching)
	{
		return;
...
void AA_QueueEffect::AttachToContainer()
{
	if (!bIsAttaching)
	{
		return;
...
void AA_QueueEffect::MoveNodesConsideringOrder()
{
...
void AA_QueueEffect::RefreshNameLocationAndRotation()
{
...
void AA_QueueEffect::SetContainerName(FText Name)
{
	ContainerName = Name;
	TextComp->SetText(Name);
}

AActor* AA_QueueEffect::GetActorWithTag(const FName& Tag)
{
	if (!GetWorld())
	{
		return nullptr;
...
void AA_QueueEffect::TimelineProgress(float Value)
{
...
void AA_QueueEffect::TimelineFinished()
{
